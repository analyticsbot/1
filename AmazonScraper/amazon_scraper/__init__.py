...
AUTH = requests.auth.HTTPProxyAuth('alexisperk', 'alexisperk')
...
def reviews_url(asin):
    ...
    #the following line is in case you want to get the most helpful reviews first
    url = '{base}/product-reviews/{asin}/ref=cm_cr_arp_d_viewopt_srt?sortBy=helpful&pageNumber=1'
    return url.format(base=amazon_base, asin=asin)
...
def get(url, api):
    ...
    #r = requests.get(url, proxies=PROXIES, auth=AUTH, headers={'User-Agent': random_user_agent})
    ...
    #r = requests.get(url, headers={'User-Agent': random_user_agent}, verify=False) # original code; without proxy
    r.raise_for_status()
    return r
