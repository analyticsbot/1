...
class Review(object):
    ...
    @property
    def helpful(self):
        pattern = re.compile(r"(\d+) of (\d+) people found the following review helpful")
        for elm in self.soup.find_all("div", text=pattern):
            digits = pattern.search(elm.text).groups()
            
            # the two lines below should work, but if they don't replace them with: return [str(x) for x in digits] 
            type = [x.encode('utf8') for x in digits]
            return ', '.join(type)
            #return [str(x) for x in digits]
#         text = self.soup.findAll(text=True)
#         text2 = u'\n'.join(text).strip()      
#         helpful_regex = re.compile(r'\d+ of \d+ people found the following review helpful')
#         helpful_match = helpful_regex.search(text2)
#         helpful_text = helpful_match.group(0)
#         digits = [int(s) for s in re.findall(r'\d+', helpful_text)]
#         return digits
