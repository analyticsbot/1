...
class SubReview(object):
    ...
    @property
    def helprate(self):
        pattern = re.compile(r'(\d+) people found this helpful')
        #pattern2 = re.compile(r'(\D+) person found this helpful')
        tag = self.soup.find('span', class_=re.compile('review-votes'))
        # You had to strip the text in case the digits had a comma in them (e.g., "1,000")
        if tag is not None:
            tag_stripped = tag.text.replace(',', '')
            try:
                digits = pattern.search(tag_stripped).groups()
                #type = [x.encode('utf8') for x in digits]
                return digits
            except:
                return "1" # if there is an error, I'm assuming it's because Amazon has written the word "One" there
        else:
            return "0"
        
#         pattern = re.compile(r'(\d+) of (\d+) people found this helpful')
#         tag = self.soup.find('span', class_=re.compile('review-votes'))
#         # You had to strip the text in case the digits had a comma in them (e.g., "1,000")
#         if tag is not None:
#             tag_stripped = tag.text.replace(',', '')
...
class Reviews(object):
    ...
    def total_reviews(self):
        tag = self.soup.find('span', class_=re.compile('totalReviewCount'))
        if tag:
            tag_stripped = tag.text.replace(',', '')
            return tag_stripped
        else:
