...
ignore_keywords = ['escrow', 'esq','inc','law', 'service', 'law', 'Timeshare', 'associates', 'recorder', 'recorders', '$' , 'iiii', 'service', 'closing', 'title']
...
pwd = 'billie54'
state = 'CA'
#hugo this is where I changed the county to SAN BERNADINO does not work..???, RIVERSIDE WORKS TOO!
county = 'RIVERSIDE'
#hugo this is where I change to "to:" and also add "death". It used to work with "to:" intially, now not...
keyword = '"to:" "death"'
input_file_name = ''
...
MAX_PAGES_SCRAPE = 2#'ALL'
##MAX_PAGES_SCRAPE = 'ALL'

DOCUMENT_TYPE_LEFT_ALLOWED = ["AFFIDAVIT - DEATH OF TRUSTEE"]
...
##                continue
            ...
            try:
                ...
                pass
#hugo this is where it parses the right side...works ok....
            try:
            ...
            try:
                ...
                SELLER_LENDER = 'NA'
#hugo this is the hash parse section that should comment out to prevent skiping....
            try:
                try:
...
## allen............name.......................
                ...
                logging.info("....Allen1...Record number - " + str(post_count) + "; Right hand name and address present. Parsed full address -- " + address \
                                      + '; allenName = ' + name + '; streetName = ' + streetName + '; PlaceName = '+ PlaceName + '; StateName = ' + StateName\
                                      + '; ZipCode = ' + ZipCode + '\n and hash value -- ' + hash_left_data)
                

## allen...................
                parse_address = usaddress.parse(address)
## allen this is to see the resultant of usaddress parse!!!
#hugo this is the left parse of the name and addr data on the recorded document....
#Hugo ..will need to change to look for just addr in the case of doc type of NOTICE or ORDER..I will set keyword in query to "located at"
#hugo...as a separate run instead of "to:"
                SELLER_LENDER = usaddress.parse(address)
                logging.info("....PARSE DATA - " + parse_address)
                parse_address_dict = {}
                for i in parse_address:
                    if i[1] not in parse_address_dict.keys():
                            parse_address_dict[i[1]] = i[0]
...
#allen add...for testing only....!                    
#                StreetNamePreDirectional = parse_address_dict
                is_parsed = 1
##allen.............................................................
                logging.info("....Allen word...Record number - " + str(post_count) + 'reject word---' + word)
...
##                            StateName = StateName_right
##                            ZipCode = ZipCode_right
##                            logging.info("Allen Log NA present")
##allen...........................................................................        
...
##allen...........................................................................
                    ...
                    StateName = StateName_right
                    ZipCode = ZipCode_right
                    logging.info("Allen Not parsing")
##allen...........................................................................
#                    hash_left_data = 6
                    df_bad.loc[post_count] = [cur_date, text_box_left, DOCUMENT_TYPE_LEFT, DOCUMENT_TYPE_RIGHT, RECORDING_DATE, APN, \
                                  ADDRESS, address_number_right, streetName_right, PlaceName_right, StateName_right, ZipCode_right, OWNER_BORROWER, SELLER_LENDER, data_from_left, name_address, name, address,\
...
#                        hash_left_data = 7
                        ...
                        if address==' ':
                             ...
                             [post_count] = [cur_date, text_box_left, DOCUMENT_TYPE_LEFT, DOCUMENT_TYPE_RIGHT, RECORDING_DATE, APN, \
                              ADDRESS, address_number_right, streetName_right, PlaceName_right, StateName_right, ZipCode_right, OWNER_BORROWER, SELLER_LENDER, data_from_left, name_address, name, address,\
                              address_number, streetName, StreetNamePostType, PlaceName, StateName,\
                              ZipCode, StreetNamePostType, StreetNamePreDirectional, hash_left_data, is_parsed]
                    ...
                    logging.info("Allen Log last present")
#                    hash_left_data = 9
                    df_bad.loc[post_count] = [cur_date, text_box_left, DOCUMENT_TYPE_LEFT, DOCUMENT_TYPE_RIGHT, RECORDING_DATE, APN, \
                              ADDRESS, address_number_right, streetName_right, PlaceName_right, StateName_right, ZipCode_right, OWNER_BORROWER, SELLER_LENDER, data_from_left, name_address, name, address,\
                              address_number, streetName, StreetNamePostType, PlaceName, StateName,\
...
df.to_csv(output_file_name[:-4]+'_testing.csv', encoding = 'utf-8', index = True)
...
logging.info("Output written to file : " + output_file_name)
df_bad.to_csv(output_file_name[:-4] + '_bad_data.csv', encoding = 'utf-8', index = True)
