...
df = pd.DataFrame(columns = ['date', 'text_box_left','DOCUMENT_TYPE_LEFT', 'DOCUMENT_TYPE_RIGHT', 'RECORDING_DATE', 'APN', \
                             'is_parsed','mail_parsed_name', 'mail_parsed_address',\
                             'mail_address_number', 'mail_streetName','mail_StreetNamePostType', 'mail_PlaceName', 'mail_StateName',\
                             'mail_ZipCode','mail_StreetNamePostType', 'mail_StreetNamePreDirectional',\
                             'ADDRESS', 'address_number_right', 'streetName_right', 'PlaceName_right', 'StateName_right','ZipCode_right',\
                             'OWNER_BORROWER','SELLER_LENDER', 'text', 'parsed_name_address', 'parsed_name', 'parsed_address',\
                             'address_number', 'streetName','StreetNamePostType', 'PlaceName', 'StateName',\
                             'ZipCode','StreetNamePostType', 'StreetNamePreDirectional', 'hash_value'])
...
##ALLEN....................................
            ...
            try:
                ...
                SELLER_LENDER = 'NA'
            logging.info("record number before hash -- " + str(doc_count))
            try:
                try:
                    ...
                    #print str(e), '\n'
                    data_from_left = 'NA'
                logging.info("record number after hash -- " + str(doc_count))
                if data_from_left == 'NA':
                    continue
                logging.info("record number start name address -- " + str(doc_count))
                name_address = re.findall(r'\s([a-z][a-z]+.*?)$', data_from_left)[0]
                name_address = re.findall(r'(\w\w.*?\s\w\w\.?\s*\d\d\d\d\d)', name_address)[0]
                logging.info("record number parse name address -- " + name_address+ str(doc_count))
                ...
                parse_address = usaddress.parse(address)
                SELLER_LENDER = usaddress.parse(address)
                parse_address_dict = {}
                for i in parse_address:
                    if i[1] not in parse_address_dict.keys():
                            parse_address_dict[i[1]] = i[0]
...
#                    mail_ZipCode = ZipCode_right
                ...
                try:
                    ...
                    StreetNamePreDirectional = mail_StreetNamePreDirectional
                except:
...
##                            df.loc[post_count] = [cur_date, text_box_left, DOCUMENT_TYPE_LEFT, DOCUMENT_TYPE_RIGHT, RECORDING_DATE, APN, \
##                                  ADDRESS, is_parsed, address_number_right, streetName_right, PlaceName_right, StateName_right, ZipCode_right, OWNER_BORROWER, SELLER_LENDER, data_from_left, name_address, name, address,\
##                                 address_number, streetName, StreetNamePostType, PlaceName, StateName,\
##                             ZipCode, StreetNamePostType, StreetNamePreDirectional, hash_left_data]
#                            df.loc[post_count] = ['date', 'text_box_left','DOCUMENT_TYPE_LEFT', 'DOCUMENT_TYPE_RIGHT', 'RECORDING_DATE', 'APN', \
#                                'is_parsed','mail_parsed_name', 'mail_parsed_address',\
#                             'mail_address_number', 'mail_streetName','mail_StreetNamePostType', 'mail_PlaceName', 'mail_StateName',\
#                             'mail_ZipCode','mail_StreetNamePostType', 'mail_StreetNamePreDirectional',\
#                             'ADDRESS', 'address_number_right', 'streetName_right', 'PlaceName_right', 'StateName_right','ZipCode_right',\
...
##this is start of code that needs fix/specifically the "IF ADDRESS not in ('NA, ' '):" if fails syntax rules...                            
...
##'mail_parsed_name', 'mail_parsed_address',\
##                             'mail_address_number', 'mail_streetName','mail_StreetNamePostType', 'mail_PlaceName', 'mail_StateName',\
##                             'mail_ZipCode','mail_StreetNamePostType', 'mail_StreetNamePreDirectional',\
                             
new_df = df[['OWNER_BORROWER','ADDRESS','address_number_right', 'streetName_right', 'PlaceName_right', 'StateName_right',
                             'ZipCode_right', 'mail_parsed_name', 'mail_parsed_address','mail_address_number', 'mail_streetName','mail_StreetNamePostType', 'mail_PlaceName', 'mail_StateName', 'mail_ZipCode','mail_StreetNamePostType', 'mail_StreetNamePreDirectional', 'parsed_name', 'parsed_address', 'address_number', 'streetName','StreetNamePostType', 'PlaceName', 'StateName',\
