...
ignore_keywords = ['escrow', 'esq', 'llp', 'law', 'associates', 'law', 'service', 'recorder', 'recorders', '$' , 'iiii', 'service', 'closing', 'title']
...
MAX_PAGES_SCRAPE = 1#'ALL'
DOCUMENT_TYPE_LEFT_ALLOWED = ['AFFIDAVIT OF DEATH']
#DOCUMENT_TYPE_LEFT_ALLOWED = ['NOTICE OF ABATEMENT', 'NOTICE OF LIEN', 'NOTICE OF DEFAULT/ELECTION TO SELL ASSESSMENT', 'TRANSFER', 'NOTICE OF CODE VIOLATION', 'NOTICE OF COMPLIANCE',]
...
doc_types = ['UNDETERMINED', 'DEED', 'NOTICE']
"""
POSSIBLE VALUES
                ...
                if not any(word in text_box_left.lower() for word in ignore_keywords):
                    if ((OWNER_BORROWER.strip() not in ['NA','']) or (ADDRESS.strip() not in ['NA',''])):
                        if (streetName=='NA') and (PlaceName=='NA') and (StateName=='NA') and (ZipCode=='NA'):
                            ...
                            StateName = StateName_right
                            ZipCode = ZipCode_right
##                            if ((address_number not in ['NA','']) or (streetName not in ['NA','']) or (PlaceNme not in ['NA',''])): 
##                               or (PlaceName not in ['NA','']) or (StateName not in ['NA','']) or (ZipCode not in ['NA',''])):
